...
void OneCopyRasterBufferProvider::PlaybackToStagingBuffer(
  ...
  if (staging_buffer->gpu_memory_buffer) {
    ...
    if (std::string("renderer") ==
      ...
      BrokerCompressionData compression_data(BrokerCompressionMode::WEBP,
          staging_buffer->size.width(),
          staging_buffer->size.height(), buffer->stride(0));
      mojo::SyncSharedMemory(
          buffer->GetHandle().handle.GetGUID(), 0,
          staging_buffer->size.width() * staging_buffer->size.height() * 4, compression_data);
    }
#endif
    buffer->Unmap();
